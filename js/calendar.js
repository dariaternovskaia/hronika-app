...
function getEffectiveEndDate(ch) {
    ...
    if (today.getFullYear() !== year) return { day: 31, month: 11, year: year };
...
async function renderCalendar() {
    ...
    html += `<div style="margin-bottom:16px;display:flex;justify-content:flex-end;gap:8px;">
        ...
        <label style="background:#1a2230;color:#7a8ba8;border:1px solid #1f2838;border-radius:12px;padding:10px 16px;font-size:13px;cursor:pointer;font-family:inherit;">📤 Импорт<input type="file" id="importFile" accept=".json" style="display:none;" /></label>
    ...
    if (challenges.length === 0) {
        html += `<div style="text-align:center;padding:40px;color:#7a8ba8;font-size:16px;">📭 Пока нет челленджей. Нажми "+ Добавить челлендж" чтобы создать первый.</div>`;
    }
    ...
    for (let m = 0; m < 12; m++) {
        ...
        html += `</div><div class="challenge-bars" id="bars-${m}"></div></div></div>`;
    }

    container.innerHTML = html;

    document.getElementById('addChallengeBtn').addEventListener('click', () => openChallengeModal());
    document.getElementById('exportBtn').addEventListener('click', exportChallenges);
    document.getElementById('importFile').addEventListener('change', function(e) {
        if (e.target.files.length > 0) importChallenges(e.target.files[0]);
    });

    requestAnimationFrame(() => {
        requestAnimationFrame(() => renderChallengeBars());
    });
}
    ...
    for (let m = 0; m < 12; m++) {
        ...
        challenges.forEach(ch => {
            ...
            monthChallenges.push({
                challenge: ch, sDay, eDay,
                isClippedRight: (ch.endDate === null),
                isClippedLeft: (m > sM)
            });
        });

        const rows = [];
        const barHeight = 26, labelHeight = 16, rowGap = 8;
        monthChallenges.forEach(item => {
            let placed = false;
            for (let r = 0; r < rows.length; r++) {
                let overlap = false;
                for (const existing of rows[r]) {
                    if (item.sDay <= existing.eDay && item.eDay >= existing.sDay) { overlap = true; break; }
                }
                if (!overlap) { rows[r].push(item); placeBar(item, r); placed = true; break; }
            }
            if (!placed) { rows.push([item]); placeBar(item, rows.length - 1); }
        });
        ...
        function placeBar(item, rowIndex) {
            ...
            label.style.cssText = `position:absolute;left:${(item.sDay-1)*cellStep}px;top:${rowIndex*(barHeight+labelHeight+rowGap)}px;font-size:10px;color:#7a8ba8;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:${(item.eDay-item.sDay+1)*cellStep}px;`;
            ...
            bar.style.left = (item.sDay - 1) * cellStep + 'px';
            bar.style.top = (rowIndex * (barHeight + labelHeight + rowGap) + labelHeight) + 'px';
            bar.style.width = ((item.eDay - item.sDay + 1) * cellWidth + (item.eDay - item.sDay) * gap) + 'px';
            if (item.isClippedRight) { bar.style.borderTopRightRadius = '0'; bar.style.borderBottomRightRadius = '0'; bar.style.borderRight = '2px dashed rgba(255,255,255,0.3)'; }
            if (item.isClippedLeft) { bar.style.borderTopLeftRadius = '0'; bar.style.borderBottomLeftRadius = '0'; bar.style.borderLeft = '2px dashed rgba(255,255,255,0.3)'; }
...
function openChallengeModal(id = null) {
    ...
    const contentAmount = ch?.content?.amount ?? '';
    const contentUnit = ch?.content?.unit ?? 'pages';
    const contentCustomUnit = ch?.content?.customUnit ?? '';
    const contentNote = ch?.content?.note ?? '';
    const normAmount = ch?.norm?.amount ?? '';
    const normUnit = ch?.norm?.unit ?? contentUnit;
    const freqType = ch?.frequency?.type ?? 'daily';
    const freqDays = ch?.frequency?.days ?? [];
    ...
    const html = `
        <div class="modal-overlay active" id="challengeModal">
            <div class="modal-card">
                <h2>${title}</h2>
                <div style="margin-bottom:12px;"><label style="display:block;font-size:13px;color:#7a8ba8;margin-bottom:6px;">Название</label>
                <input type="text" id="chName" value="${name}" style="width:100%;padding:10px 14px;border-radius:10px;border:1px solid #1f2838;background:#0b0e14;color:#e8edf5;font-size:14px;font-family:inherit;outline:none;" /></div>
                <div style="margin-bottom:12px;"><label style="display:block;font-size:13px;color:#7a8ba8;margin-bottom:6px;">Комментарий</label>
                <textarea id="chComment" style="width:100%;padding:10px 14px;border-radius:10px;border:1px solid #1f2838;background:#0b0e14;color:#e8edf5;font-size:14px;font-family:inherit;outline:none;min-height:60px;resize:vertical;">${comment}</textarea></div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
                    <div><label style="display:block;font-size:13px;color:#7a8ba8;margin-bottom:6px;">Дата начала</label>
                    <input type="date" id="chStart" value="${ch ? formatDateForInput(ch.startDate) : ''}" style="width:100%;padding:10px 14px;border-radius:10px;border:1px solid #1f2838;background:#0b0e14;color:#e8edf5;font-size:14px;font-family:inherit;" /></div>
                    <div><label style="display:block;font-size:13px;color:#7a8ba8;margin-bottom:6px;">Дата окончания</label>
                    <input type="date" id="chEnd" value="${ch?.endDate ? formatDateForInput(ch.endDate) : ''}" ${!hasEndDate ? 'disabled' : ''} style="width:100%;padding:10px 14px;border-radius:10px;border:1px solid #1f2838;background:#0b0e14;color:#e8edf5;font-size:14px;font-family:inherit;${!hasEndDate ? 'opacity:0.5;' : ''}" />
                    <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:#7a8ba8;margin-top:6px;cursor:pointer;"><input type="checkbox" id="chNoEnd" ${!hasEndDate ? 'checked' : ''} /> Без даты окончания</label></div>
                </div>
                ...
                <div style="background:#0b0e14;padding:12px;border-radius:10px;border:1px solid #1f2838;margin-bottom:12px;">
                    ...
                    <div id="weekDaysContainer" style="display:${freqType==='weekly'?'block':'none'};">
                        <div style="display:flex;gap:4px;flex-wrap:wrap;">
                            ${weekDays.map((day, idx) => `<label style="display:flex;align-items:center;gap:4px;padding:6px 10px;background:#141a24;border-radius:8px;cursor:pointer;font-size:12px;color:#e8edf5;"><input type="checkbox" class="weekDayCheckbox" value="${idx}" ${freqDays.includes(idx)?'checked':''} style="cursor:pointer;" />${day}</label>`).join('')}
                        </div>
                    </div>
                </div>
                <div style="margin-bottom:12px;"><label style="display:block;font-size:13px;color:#7a8ba8;margin-bottom:6px;">Цвет</label>
                <input type="color" id="chColor" value="${color}" style="width:60px;height:40px;border:1px solid #1f2838;border-radius:8px;background:#0b0e14;cursor:pointer;" /></div>
    ...
    document.getElementById('saveChBtn').addEventListener('click', () => {
        ...
        if (!noEnd && !document.getElementById('chEnd').value) { alert('Выберите дату окончания или отметьте "Без даты окончания"'); return; }
        ...
        const freqDays = freqType === 'weekly' ? Array.from(document.querySelectorAll('.weekDayCheckbox:checked')).map(cb => parseInt(cb.value)) : [];
        ...
        const challenge = {
            ...
            startDate, endDate,
            content: { amount: parseInt(document.getElementById('contentAmount').value) || 0, unit: contentUnit, customUnit: contentCustomUnit, note: document.getElementById('contentNote').value.trim() },
            norm: { amount: parseInt(document.getElementById('normAmount').value) || 0, unit: document.getElementById('normUnit').value },
...
function exportChallenges() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `hronika-challenges-${new Date().toISOString().split('T')[0]}.json`; a.click();
